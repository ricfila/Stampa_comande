...
def stampa_singola(app, template):
	...
	if not id.isdigit():
		...
		return
	if os.path.isfile(render.file_comanda(id, "cliente")):
		if os.path.isfile(render.file_comanda(id, template)):
...
def stampa_comanda(id, tipo, app, manuale = False):
	output_file = render.file_comanda(id, tipo)
	params = [output_file, configs['Stampa']['stampante'], formato_carta]
	#si = subprocess.STARTUPINFO()
	#si.dwFlags |= subprocess.STARTF_USESHOWWI
	result = subprocess.run([printer_executable] + params, capture_output=True, text=True)#, startupinfo=si)
